...
namespace ReaderWriterLock
{
    ...
    public class MyReaderPreferenceRwLockTests : RwLockTests<ReaderPreferenceRwLock>
    {
    }

    public class MyWithoutWriterSpinRwLockTests : RwLockTests<ReaderPreferenceWithoutWriterSpinRwLock>
    {
    }

    public class MyWriterPreferenceRwLockTests : RwLockTests<WriterPreferenceRwLock>
    {
    }
    ...
    public abstract class RwLockTests<T> where T : IRwLock, new()
    {
        ...
        public void TestStress(int readersCount, int writersCount, int readTimeout, int writeTimeout)
        {
            ...
            var readThreads = Enumerable.Range(0, readersCount)
                .Select(_ => new Thread(_ => ReadIndefinitely(cts.Token, readTimeout))).ToArray();
            var writeThreads = Enumerable.Range(0, writersCount)
                .Select(_ => new Thread(_ => WriteIndefinitely(cts.Token, writeTimeout))).ToArray();
            ...
            Console.WriteLine($"Reads/Writes: {reads / (double) writes}");
        }
